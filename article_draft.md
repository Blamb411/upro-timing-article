...
**Price returns vs total returns.** This analysis uses price returns only (dividends excluded for SPY and related calculations). This is a conservative assumption -- including SPY dividends (~1.3% annual yield) would improve the SPY benchmark by roughly 1.5-2% CAGR annually, making the comparison *more favorable to SPY* relative to the timing strategies. UPRO itself targets 3x the daily *price* return of the S&P 500 (before fees) and does not distribute dividends, so price returns are the appropriate apples-to-apples comparison for the UPRO-specific results.
